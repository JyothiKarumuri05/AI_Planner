...
app.post("/save-travel", async (req, res) => {
  try {
    const travelData = req.body;
    ...
    pythonProcess.on("close", async () => {
      const aiResult = JSON.parse(resultData.trim());

      await pool.query(
        `INSERT INTO travel_plans (user_id, request_id, final_itinerary)
         VALUES ($1,$2,$3)`,
        [travelData.user_id, requestId, aiResult.itinerary]
      );
...
app.get("/travel-history/:userId", async (req, res) => {
  try {
    const { userId } = req.params;
...
app.get("/download-pdf/:request_id", async (req, res) => {
  try {
    const requestId = req.params.request_id;
